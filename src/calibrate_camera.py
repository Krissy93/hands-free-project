...
def findRT(img, mtx, dist):
    ...
    # we have used a chessboard of 10 squares along x and 7 squares along y
...
def initial_calibration(path, Debug=False):
    ...
    objp = np.zeros((6*9, 3), np.float32)
    objp[:, :2] = np.mgrid[0:9, 0:6].T.reshape(-1, 2) * 22.5  # 22.5mm chessboard square size
    ...
    print(utils.Color.BOLD + utils.Color.CYAN + "Total error of calibration: " + str(mean_error/len(objpoints)) + utils.Color.END)

    return mtx, dist


def undistort_image(img, K, D, path):
    ...
    x, y, w, h = roi
    dst = dst[y:y+h, x:x+w]
    cv2.imwrite(path+'/hands_free/src/debug/undistorted_image.png', dst)
...
def main():
    ...
    STEP 3: crop and show both images
    ...
    ######### STEP 1
    # Gives as input the initial path of the folder containing the calibration images
    K, D = initial_calibration(path+'/hands_free/src/calib_img/', Debug=Debug)

    ######### STEP 2
    camera = utils.Camera(True)
    camera.acquire()
    # saves the frame as a copy, needed to correctly work with cv images
    img = camera.color_new.copy()
    # stops the device since only a snapshot was needed
    camera.stop()
    # saves the acquired frame on disk, useful to debug
    cv2.imwrite(path+'/hands_free/src/debug/workspace_image.png', img)

    dst = undistort_image(img, K, D, path)
    ...
    if Debug:
        ...
        # to check the cut, show the image dst
        cv2.imshow('Cropped Image UNDISTORTED', dst)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    ...
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imwrite(path+'/hands_free/src/debug/original_reference.png', img)
    ...
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imwrite(path+'/hands_free/src/debug/undistorted_reference.png', dst)

    ######### STEP 6
    dictionary = [{'K': K.tolist()}, {'D': D.tolist()}, {'R': R1.tolist()}, {'t': tvecs1.tolist()}, {'Rd': R2.tolist()}, {'td': tvecs2.tolist()}]
    utils.dict2yaml(dictionary, path+'/hands_free/src/yaml/camera_calibration.yaml')
