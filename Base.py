...
# ok
def hand_keypoints(net, frame, threshold, inHeight, nPoints):
    ...
    print(inpBlob.shape)
    # actual detection using the input blob and the network
    #net.setInput(inpBlob)
    print(net.blobs['image'].data)
    net.blobs['image'].data[0,...] = inpBlob
    output = net.forward()
...
# ok
def gesture(points, chain, acquire):
    ...
    if sum(handmap) == 5:
        ...
        G = 'INDEX'
        if acquire == True and len(chain) < 10:
            # estrae 10 volte le coordinate xyz dell'indice, coordinata 8
            chain.append(points[8])
    else:
        rospy.loginfo(color.BOLD + color.RED + 'NO GESTURE' + color.END)
        G = 'NO GESTURE'
        # non svuoto perche' accetto il disturbo
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        points, inference_time = hand_keypoints(net, kinect.color_new, threshold, inHeight, nPoints)
        # if all(x == 'None' for x in points) or points[0] == None:
        #     rospy.loginfo(color.BOLD + color.RED + 'NO GESTURE FOUND IN IMAGE' + color.END)
        #     draw = False
        #     G = 'NO GESTURE'
        # else:
        #     chain, acquire, G = gesture(points, chain, acquire)
        #     draw = True
        #     if len(chain) == 10:
        #         # ho riempito la coda 10 volte
        #         acquire = False
